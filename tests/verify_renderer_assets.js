...
assert(app.includes('class="library-project-card-tags${projectTags') && /\.library-project-title\s*\{[^}]*-webkit-line-clamp:\s*2/s.test(css), 'Media-project cards must show compact tags below a two-line title');
...
assert(/\.library-provider-badge\s*\{[^}]*top:\s*7px[^}]*left:\s*7px[^}]*border-radius:\s*4px[^}]*background:\s*#15803d/s.test(css), 'Media-project source badges must use a smaller inset treatment');
assert(/\.library-grid\s*\{[^}]*minmax\(210px,\s*1fr\)/s.test(css) && app.includes("is-${isProjects ? 'projects' : 'assets'}"), 'Media projects and all-assets cards must share the more readable grid width');
